...
function http(e) {
	...
	function doit(method, url, headers, param, callback, errorCallback) {
		...
		axios({
			...
			xsrfCookieName: '__Host-csrf', // 기본값
		
			// `xsrfHeaderName`은 xsrf 토큰 값을 운반하는 HTTP 헤더의 이름입니다.
			xsrfHeaderName: 'x-csrf-token', // 기본값
			...
			onUploadProgress: function (progressEvent) {
				...
				if(!!e.thisVar)
				{
					if(!!e.thisVar.uploadProgressValue)
					{
						e.thisVar.uploadProgressValue = `${progressEvent.loaded} / ${progressEvent.total} (메시지 바이트 수)`
						let el = document.querySelector('#progress')
						if(!!el)
							el.setAttribute('value', Math.floor(progressEvent.loaded / progressEvent.total * 100))
					}
				}
			},
		
			// `onDownloadProgress`는 다운로드 진행 이벤트를 처리합니다.
			// 브라우저 전용
  ...
  try {
    // 크리덴셜 자동 주입
    if(e.uri.startsWith('/api/v2') || e.uri === '/admin/whoami')
    {
      if(e.param instanceof FormData)
      {
      }
      else
      {
    ...
    e.param.origin = 'localhost-1'
  
    if(!e.cb) e.cb = (res) => {}
  } catch (e) {
    console.error("크리덴셜 자동 주입 중에 오류가 발생하였습니다.\n", e)
  }
	// URL 변환
	switch(e.uri)
	{
		...
		case '이미지게시판쓰기':
		{
			e.uri = '/api/v2/image-table'
			let formData = new FormData()
			for(let row of e.param)
			{
				if(!!row.fileext)
					formData.append('part', 'I' + row.id.toString().padStart(2, '0') + row.content)
				else
					formData.append('part', 'T' + row.id.toString().padStart(2, '0') + row.content)
			}
			formData.append('tableName', 'TB_IM_NOTICE1')
			e.param = formData
			e.headers = {
  ...
  doit(e.method, e.uri, e.headers, e.param, e.cb, e.ecb)
}